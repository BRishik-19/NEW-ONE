...
def update():
    ...
    new= Functions.get_todos(r"webapps\todos_items.txt")
    new.append(new_todo)

    Functions.write_todos(new, r"webapps\todos_items.txt")

st.title("My todo app")
st.subheader("This is my new app")
st.write("A simple app adding,editing,completing todos items...")
todos= Functions.get_todos(r"webapps\todos_items.txt")

for ind,i in enumerate(todos):
    checkbox=st.checkbox(i,key=i)
    if checkbox:
        todo= Functions.get_todos(r"webapps\todos_items.txt")
        todo.pop(ind)
        Functions.write_todos(todo, r"webapps\todos_items.txt")
